...
var popups = {0:call_scene, 1:message_scene}
...
func _on_Timer_timeout():
	if get_children().size() <= 7 && !get_parent().game_over:
...
func get_rand_position():
	var rand_pos = Vector2(rand_range(150, OS.window_size.x - 150), rand_range(150, OS.window_size.y - 150))
	...
	while get_world_2d().get_direct_space_state().intersect_point(rand_pos, 32, [], 2147483647, true, true):
		rand_pos = Vector2(rand_range(150, OS.window_size.x - 150), rand_range(150, OS.window_size.y - 150))
		tries += 1
		if tries >= 20:
			break
